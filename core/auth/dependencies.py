...
from core.auth.keep_secret import read_token
...
async def get_current_user(
    ...
    try:
        ...
        if not credentials:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="No credenciales precentadas"
            )
        
        user_data = await read_token(credentials.credentials.split(",")[0])
